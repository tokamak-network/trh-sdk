...
func RestartStatefulSets(ctx context.Context, l *zap.SugaredLogger, namespace, stsCSV string) error {
	return (&BackupClient{}).restartStatefulSets(ctx, l, namespace, stsCSV)
}
...
func findStatefulSetByName(ctx context.Context, namespace, pattern string) (string, error) {
	return (&BackupClient{}).findStatefulSetByName(ctx, namespace, pattern)
}
...
func ExecuteBackupAttach(
	...
	if attachInfo.EFSID != "" {
		...
		_ = (&BackupClient{}).k8sDeletePod(ctx, "verify-efs", attachInfo.Namespace)
	}
...
func UpdatePVVolumeHandles(ctx context.Context, l *zap.SugaredLogger, namespace, newEfs string, pvcs *string) error {
	return (&BackupClient{}).updatePVVolumeHandles(ctx, l, namespace, newEfs, pvcs)
}
...
func ExecuteEFSOperationsFull(
	...
	return (&BackupClient{}).executeEFSOperationsFull(ctx, l, attachInfo, verify, backupPvPvcFlag, progressReporter)
}
...
func VerifyEFSDataImpl(ctx context.Context, l *zap.SugaredLogger, namespace string) error {
	return (&BackupClient{}).verifyEFSDataImpl(ctx, l, namespace)
}
