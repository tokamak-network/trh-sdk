...
func injectUSDCIntoGenesis(genesisPath, deploymentPath string) error {
	...
	// Load bytecodes from tokamak-thanos pre-extracted files
	implBytecodeFile := filepath.Join(deploymentPath, "tokamak-thanos", "packages", "tokamak", "contracts-bedrock", "bytecode", "l2", "FiatTokenV2_2.json")
	proxyBytecodeFile := filepath.Join(deploymentPath, "tokamak-thanos", "packages", "tokamak", "contracts-bedrock", "bytecode", "l2", "FiatTokenV2_2Proxy.json")

	implBytecode, err := loadBytecodeFromFile(implBytecodeFile)
	if err != nil {
		return fmt.Errorf("failed to load FiatTokenV2_2 implementation bytecode: %w", err)
	}

	proxyBytecode, err := loadBytecodeFromFile(proxyBytecodeFile)
	if err != nil {
		return fmt.Errorf("failed to load FiatTokenV2_2Proxy bytecode: %w", err)
	}
