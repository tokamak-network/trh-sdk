...
func (t *ThanosStack) UninstallMonitoring(ctx context.Context) error {
	...
	// Clean up AlertManager Secret
	if err := t.deleteAlertManagerSecret(ctx, monitoringNamespace); err != nil {
		fmt.Printf("⚠️  Warning: Failed to delete AlertManager secret: %v\n", err)
		// Continue anyway - this is cleanup, not critical
	}
...
func (t *ThanosStack) generateGrafanaStorageConfig(ctx context.Context, config *MonitoringConfig) map[string]interface{} {
	...
	if config.EnablePersistence {
		...
		existingClaimName := fmt.Sprintf("%s-%s-thanos-stack-grafana", config.ChainName, timestamp)
